  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        executorSource.METRIC_PUSH_BASED_SHUFFLE_BLOCKS_NOT_PUSHED
          .inc(task.metrics.shuffleWriteMetrics.blocksNotPushed)
        executorSource.METRIC_PUSH_BASED_SHUFFLE_BLOCKS_COLLIDED
          .inc(task.metrics.shuffleWriteMetrics.blocksCollided)
        executorSource.METRIC_PUSH_BASED_SHUFFLE_BLOCKS_TOO_LATE
