...
private[spark] object JsonProtocolSuite extends Assertions {
  ...
  private def makeTaskMetrics(
    ...
    if (hasOutput) {
      ...
      sw.incBlocksNotPushed(math.abs(a - b))
      sw.incBlocksCollided(math.abs(a - b))
      sw.incBlocksTooLate(math.abs(a - b))
    }
  ...
  private val taskEndJsonString =
    """
      ...
      |      "Push Based": {
      |        "Shuffle Blocks Not Pushed" : 100,
      |        "Shuffle Blocks Collided" : 100,
      |        "Shuffle Blocks Too Late" : 100
      |      }
      |    },
      |    "Input Metrics" : {
      |      "Bytes Read" : 0,
  ...
  private val taskEndWithOutputJsonString =
    """
      ...
      |      "Push Based": {
      |        "Shuffle Blocks Not Pushed" : 0,
      |        "Shuffle Blocks Collided" : 0,
      |        "Shuffle Blocks Too Late" : 0
      |      }
      |    },
      |    "Input Metrics": {
      |      "Bytes Read": 2100,
  ...
  private val executorMetricsUpdateJsonString =
    s"""
      ...
      |        {
      |          "ID": 19,
      |          "Name": "${shuffleRead.ORIGINAL_BLOCK_FALLBACK_COUNT}",
      |          "Update": 0,
      |          "Internal": true,
      |          "Count Failed Values": true
      |        },
      ...
      |        {
      |          "ID": 31,
      |          "Name": "${shuffleWrite.BLOCKS_NOT_PUSHED}",
      |          "Update": 0,
      |          "Internal": true,
      |          "Count Failed Values": true
      |        },
      |        {
      |          "ID": 32,
      |          "Name": "${shuffleWrite.BLOCKS_COLLIDED}",
      |          "Update": 0,
      |          "Internal": true,
      |          "Count Failed Values": true
      |        },
      |        {
      |          "ID": 33,
      |          "Name": "${shuffleWrite.BLOCKS_TOO_LATE}",
      |          "Update": 0,
      |          "Internal": true,
      |          "Count Failed Values": true
      |        },
